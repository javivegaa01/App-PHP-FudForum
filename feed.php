...
function smiley_full(&$data)
{
	...
	switch ($mode) {
		case 'm':
			...
			while ($r = db_rowobj($c)) {
				...
				if ($format == 'rdf') {
					$feed_header .= '<rdf:li rdf:resource="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'" />';
	...
	<content:item rdf:about="poll_opt">
		...
		<opt_votes>'.$o[1].'</opt_votes>
					...
					$feed_data .= ($basic ? '
<item rdf:about="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'">
	<title>'.htmlspecialchars($r->subject).'</title>
	<link>http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'</link>
...
' ) ;
	...
	<title>'.htmlspecialchars($r->subject).'</title>
	<link href="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'" />
	<id>http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'</id>
...
' ) .'
<channel rdf:about="http://www.javiervega-fudforum.org/index.php">
	<title>'.$FORUM_TITLE.' - Suministro RDF</title>
	<link>http://www.javiervega-fudforum.org/index.php</link>
...
<rss version="2.0">
<channel>
	<title>'.$FORUM_TITLE.' - Suministro RSS2</title>
	<link>http://www.javiervega-fudforum.org/index.php</link>
	<description>'.sp($GLOBALS['FORUM_DESCR']).'</description>
...
<feed xmlns="http://www.w3.org/2005/Atom">
	<title>'.$FORUM_TITLE.' - Suministro Atom</title>
	<subtitle>'.sp($GLOBALS['FORUM_DESCR']).'</subtitle>
	<link href="http://www.javiervega-fudforum.org/index.php" />
	<updated>'.gmdate('Y-m-d\TH:i:s', __request_timestamp__).'-00:00</updated>
	<id>http://www.javiervega-fudforum.org/index.php</id>
	...
	'.$feed_data.'
		...
		case 't':
			...
			while ($r = db_rowobj($c)) {
				...
				if ($format == 'rdf') {
					$feed_header .= '<rdf:li rdf:resource="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;th='.$r->id.'" />';
					$feed_data .= ($basic ? '
<item rdf:about="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;th='.$r->id.'">
	<title>'.htmlspecialchars($r->subject).'</title>
	<link>http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;th='.$r->id.'</link>
	<description>'.sp($body).'</description>
...
' ) ;
	...
	<link href="http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;th='.$r->id.'" />
	<id>http://www.javiervega-fudforum.org/index.phpindex.php?t=rview&amp;th='.$r->id.'</id>
...
' : '' ) .'
	...
	<link>http://www.javiervega-fudforum.org/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'</link>
	<author>'.sp($r->name).'</author>
	'.($r->last_visit ? '<pubDate>'.gmdate('Y-m-d\TH:i:s', $r->last_visit).'</pubDate>' : '' ) .'
</item>';
				if ($format == 'atom') $feed_data .= '<entry>
	<title>'.sp($r->alias).'</title>
	<link href="http://www.javiervega-fudforum.org/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'" />
	<id>http://www.javiervega-fudforum.org/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'</id>
	<author>
		<name>'.sp($r->name).'</name>
...
<errors>
	<error>
		<message>No se encontró ningún dato coincidente.</message>
	</error>
</errors>');
	}
?>
