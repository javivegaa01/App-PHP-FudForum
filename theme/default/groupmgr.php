...
function draw_tmpl_perm_table($perm, $perms, $names)
{
	...
	foreach ($perms as $k => $v) {
		$str .= ($perm & $v[0]) ? '<td title="'.$names[$k].'" class="permYES">Sí</td>' : '<td title="'.$names[$k].'" class="permNO">No</td>';
	}
	return $str;
}
...
<tr>
	<th colspan="3">Selección de editor de grupo</th>
</tr>
<tr class="RowStyleC">
	<td class="nw fb">Grupo:</td>
	<td><select name="group_id">'.tmpl_draw_select_opt(rtrim($vl), rtrim($kl), $group_id).'</select></td>
	<td class="ar"><input type="submit" class="button" name="btn_groupswitch" value="Editar grupo" /></td>
</tr>
</table></div>'._hs.'</form>';
	}
...
/* Print number of unread private messages in User Control Panel. */
	...
	if (!$grp->forum_id) {
		$group_resources = '<b>Este grupo controla los permisos de los foros siguientes:</b><br />';
		...
		$group_resources = '<b>Grupo principal para los foros:</b> '.$fname;
	}
	...
	if (isset($_POST['btn_submit'])) {
		...
		if (empty($_POST['edit'])) {
			...
			if (!($usr_id = q_singleval('SELECT id FROM fud30_users WHERE alias='. _esc(char_fix(htmlspecialchars($gr_member)))))) {
				$login_error = '<span class="ErrorText">No existe un usuario con el nombre "'.char_fix(htmlspecialchars($gr_member)).'"</span><br />';
			} else if (q_singleval('SELECT id FROM fud30_group_members WHERE group_id='. $group_id .' AND user_id='. $usr_id)) {
				$login_error = '<span class="ErrorText">El usuario "'.char_fix(htmlspecialchars($gr_member)).'" ya existe en este grupo.</span><br />';
			} else {
	...
	if (isset($_GET['edit']) && ($edit = (int)$_GET['edit'])) {
		...
		if ($mbr->user_id == 0) {
			$gr_member = '<span class="anon">Anónimo</span>';
		} else if ($mbr->user_id == '2147483647') {
			$gr_member = '<span class="reg">Todos los usuarios registrados</span>';
		} else {
			$gr_member = $mbr->alias;
		}
...
'p_EDIT'=>'Editar',
'p_DEL'=>'Borrar',
'p_STICKY'=>'Mensajes fijos',
'p_POLL'=>'Crear encuestas',
'p_FILE'=>'Adjuntar archivos',
'p_VOTE'=>'Votar',
'p_RATE'=>'Evaluar hilos/miembros',
'p_SPLIT'=>'Dividir hilos de discusión',
'p_LOCK'=>'Bloquear hilos de discusión',
'p_MOVE'=>'Trasladar temas',
'p_SML'=>'Usar emoticonos',
'p_IMG'=>'Usar etiquetas de imágenes',
'p_SEARCH'=>'Puede buscar');
...
<select name="'.$k.'" class="SmallText">
	...
	<option value="'.$v[0].'"'.$selyes.'>Sí</option>
</select>
</td>';
		} else {
...
<tr class="RowStyleB">'.$perm_select.'</tr>';
	...
	while ($obj = db_rowobj($r)) {
		...
		if ($obj->user_id == '0') {
			$member_name = '<span class="anon">Anónimo</span>';
			$group_members_list .= '<tr class="'.alt_var('mem_list_alt','RowStyleA','RowStyleB').'">
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="/index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Editar</a>]</td>
</tr>';
		} else if ($obj->user_id == '2147483647')  {
			$member_name = '<span class="reg">Todos los usuarios registrados</span>';
			$group_members_list .= '<tr class="'.alt_var('mem_list_alt','RowStyleA','RowStyleB').'">
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="/index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Editar</a>]</td>
	...
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="/index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Editar</a>] [<a href="/index.php?t=groupmgr&amp;'._rsid.'&amp;del='.$obj->user_id.'&amp;group_id='.$obj->group_id.'&amp;SQ='.$GLOBALS['sq'].'">Borrar</a>]</td>
</tr>';
			}
		}
	}
...
<br />
<div class="ac">Actualmente editando: <b><?php echo $grp->name; ?></b><br /><?php echo $group_resources; ?></div>
<br />
<form method="post" action="/index.php?t=groupmgr" id="groupmgr">
<table cellspacing="1" cellpadding="2" class="ContentTable">
<?php echo ($edit ? '
<tr class="RowStyleA">
	<td class="nw fb">Miembro</td>
...
<tr class="RowStyleA">
	<td class="nw fb">Miembro</td>
	<td class="wa al">'.$login_error.'<input tabindex="1" type="text" name="gr_member" value="'.(isset($_POST['gr_member']) ? char_fix(htmlspecialchars($_POST['gr_member'])).'' : '' )  .'" />'.($FUD_OPT_1 & (8388608|4194304) ? '&nbsp;&nbsp;&nbsp;[ <a href="javascript://" onclick="javascript: window_open(\'/index.php?t=pmuserloc&amp;'._rsid.'&amp;js_redr=groupmgr.gr_member&amp;overwrite=1\', \'user_list\',400,250);">Buscar usuario</a> ]' : '' )  .'</td>
</tr>
'); ?>
<tr class="RowStyleB">
	<td colspan="2">
...
<tr>
	<td colspan="2" class="RowStyleC ar">
		<?php echo ($edit ? '<input type="submit" tabindex="3" class="button" name="btn_cancel" value="Cancelar" /> <input type="submit" tabindex="2" class="button" name="btn_submit" value="Actualizar miembro" />' : '<input type="submit" tabindex="2" class="button" name="btn_submit" value="Agregar miembro" />'); ?>
	</td>
</tr>
</table>
...
<tr>
	<th>Miembro</th>
	<th colspan="<?php echo count($hdr); ?>">Permisos <font size="-1">(mueva el ratón por encima del permiso para ver el tipo de permiso)</font></th>
