...
function trim_body($body)
{
	...
	$TITLE_EXTRA = ': Navegador de mensajes';

	ses_update_status($usr->sid, 'Examinar mensajes usando el <a href="/index.php?t=mnav">Navegador de mensajes</a>');
...
<tr>
	<td class="GenTextRed">Debes proporcionar un intervalo de fechas válido. Este valor puede contener una coma decimal (0,12), pero debe ser mayor que cero.</td>
</tr>
</table>
</div>';
	} else if ($unit <= 0) {
...
<table cellspacing="1" cellpadding="2" class="mnavWarnTbl">
<tr>
	<td class="GenTextRed">Debe especificar una unidad de tiempo válida.</td>
</tr>
</table>
</div>';
...
<table cellspacing="1" cellpadding="2" class="mnavWarnTbl">
<tr>
	<td class="GenTextRed">El intervalo de fechas especificado es mayor del permitido por el administrador. Prueba con un intervalo más pequeño.</td>
</tr>
</table>
</div>';
...
<table cellspacing="0" cellpadding="0" class="ContentTable">';
		while ($r = db_rowobj($c)) {
			if ($oldf != $r->forum_id) {
				$mnav_data .= '<tr><th colspan="3"> Foro: <a class="thLnk" href="/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'"><span class="lg">'.filter_var($r->forum_name, FILTER_SANITIZE_STRING).'</span></a></th></tr>';
				$oldf = $r->forum_id;
			}
			if ($oldt != $r->thread_id) {
				$mnav_data .= '<tr><th class="RowStyleC">&nbsp;&nbsp;&nbsp;</th><th colspan="2"> Hilo de discusión: <a class="thLnk" href="/index.php?t='.d_thread_view.'&amp;goto='.$r->id.'&amp;'._rsid.'#msg_'.$r->id.'">'.$r->subject.'</a></th></tr>';
				$oldt = $r->thread_id;
			}
			$mnav_data .= '<tr>
	...
	<td>
		...
		<tr class="mnavH">
			...
			<td class="TopBy wa ac">Publicado por: '.(!empty($r->poster_id) ? '<a href="/index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.filter_var($r->alias, FILTER_SANITIZE_STRING).'</a>' : $GLOBALS['ANON_NICK'].'' ) .'</td>
			<td class="DateText nw ar">'.strftime('%a, %d %B %Y %H:%M', $r->post_stamp).'</td>
		</tr>
		<tr class="mnavM SmallText">
			<td colspan="3">'.trim_body(read_msg_body($r->foff, $r->length, $r->file_id)).' <a href="/index.php?t='.d_thread_view.'&amp;goto='.$r->id.'&amp;'._rsid.'#msg_'.$r->id.'">Más &raquo;&raquo;</a></td>
		</tr>
		</table>
	</td>
</tr>';
		...
		if (!$total) {
			$mnav_data = '<div class="GenText mnavNoRes ac"><p>No hay mensajes que coincidan con el criterio de búsqueda.</p></div>';
		} else {
			$mnav_data .= '</table>
</div>';
...
<tr>
	<th colspan="4" class="wa">Navegador de mensajes</th>
</tr>
<tr class="<?php echo alt_var('color_alt','RowStyleA','RowStyleB'); ?>">
	<td class="GenText nw" width="30%">Intervalo de fechas:</td>
	<td class="GenText SmallText">más reciente que<br /><input tabindex="1" type="number" name="rng" value="<?php echo $rng; ?>" maxlength="10" size="11" /></td>
	<td class="GenText SmallText">menos reciente que<br /><input tabindex="2" type="number" name="rng2" value="<?php echo $rng2; ?>" maxlength="10" size="11" /></td>
	<td class="al vb" width="60%"><select name="u" tabindex="3"><?php echo $mnav_time_unit; ?></select></td></tr>
<tr class="<?php echo alt_var('color_alt','RowStyleA','RowStyleB'); ?>">
	<td class="GenText nw">Buscar solo en:</td>
	<td colspan="3" class="vt">
		<select name="forum_limiter" tabindex="4"><option value="">Buscar en todos los foros</option>
...
<tr class="'.alt_var('color_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw">Buscar solo en los foros suscritos</td>
	<td colspan="3" class="vt"><input type="checkbox" name="sub" value="1" '.($subl ? 'checked="checked" ' : '' )  .' /></td>
' : ''); ?>
<tr class="RowStyleC">
	<td class="GenText ar" colspan="4"><input type="submit" tabindex="5" class="button" name="btn_submit" value="Comenzar a buscar" /></td>
</tr>
</table></form>
<br />
<?php echo $mnav_data; ?>
