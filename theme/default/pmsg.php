...
<tr>
	...
	ses_update_status($usr->sid, 'Usando mensajería privada');
	...
	while ($obj = db_rowobj($c)) {
		...
		if ($FUD_OPT_2 & 32 && (!($obj->users_opt & 32768) || $is_a)) {
			...
			if (($obj->time_sec + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) {
				$online_indicator = '<img src="/theme/default/images/online.png" alt="'.$obj->login.' está actualmente en el foro" title="'.$obj->login.' está actualmente en el foro" width="16" height="16" />&nbsp;';
			} else {
				$online_indicator = '<img src="/theme/default/images/offline.png" alt="'.$obj->login.' no está actualmente en el foro" title="'.$obj->login.' no está actualmente en el foro" width="16" height="16" />&nbsp;';
			}
		} else {
			$online_indicator = '';
		}
		...
		if ($obj->pmsg_opt & 64) {
			$msg_type ='<span class="SmallText">(respondido)</span>&nbsp;';
		} else if ($obj->pmsg_opt & 32) {
			$msg_type = '';
		} else {
			$msg_type ='<span class="SmallText">(reenviado)</span>&nbsp;';
		}

		$private_msg_entry .= '<tr class="RowStyleB">
	<td class="ac GenText"><input type="checkbox" name="sel[]" value="'.$obj->id.'" /></td>
	<td>'.($obj->read_stamp ? '<img src="/theme/default/images/pmsg_read.png" alt="Mensaje privado leído" width="22" height="22" title="Mensaje privado leído" />' : '<img src="/theme/default/images/pmsg_unread.png" alt="Mensaje privado no leído" width="22" height="22" title="Mensaje privado no leído" />' ) .'</td>
	<td width="100%" class="GenText">'.$msg_type.'<a href="'.$goto.'">'.$obj->subject.'</a>'.(($obj->pmsg_opt & 4 && $obj->pmsg_opt & 16 && $obj->duser_id == _uid && $obj->ouser_id != _uid) ? '<span class="SmallText">&nbsp;&nbsp;[<a href="/index.php?t=pmsg_view&amp;'._rsid.'&amp;dr=1&amp;id='.$obj->id.'" title="No enviar un mensaje confirmando al remitente que ha leído su mensaje">no enviar acuse de recibo</a>]</span>' : '' ) .'</td>
	<td class="nw GenText">'.$online_indicator.'<a href="/index.php?t=usrinfo&amp;'._rsid.'&amp;id='.$obj->ouser_id.'" title="'.char_fix(htmlspecialchars($obj->to_list)).'">'.filter_var($obj->alias, FILTER_SANITIZE_STRING).'</a></td>
	...
	if (!$private_msg_entry) {
		$private_msg_entry = '<tr class="RowStyleC"><td colspan="6" class="ac">No hay mensajes privados en esta carpeta</td></tr>';
		$private_tools = '';
	} else {
		if ($folder_id == 5) {
			$btn_action = 'Restaurar a:';
			$btn_del_name = 'btn_trash';
			$btn_del_title = 'Vaciar papelera';
		} else {
			$btn_action = 'Trasladar a:';
			$btn_del_name = 'btn_delete';
			$btn_del_title = 'Borrar';
		}
		$tmp = $folders;
		unset($tmp[$folder_id]);
		$moveto_list = tmpl_draw_select_opt(implode("\n", array_keys($tmp)), implode("\n", $tmp), 2);
		$private_tools = '<tr class="RowStyleB">
	<td class="SmallText al" colspan="3"><a href="/index.php?t=pmsg_dump&amp;'._rsid.'">Descargar volcado de mensajes</a></td>
	<td colspan="3" class="SmallText ar">
		...
		&nbsp;&nbsp;&nbsp;<input type="submit" class="button" name="'.$btn_del_name.'" value="'.$btn_del_title.'" />'.($FUD_OPT_2 & 2097152 ? '&nbsp;&nbsp;&nbsp;<input type="submit" class="button" name="btn_pdf" value="Crear archivo PDF" />' : '' )  .'
	</td>
</tr>';
	}
...
<tr>
	<td colspan="3" class="RowStyleA wa nw SmallText">Sus carpetas de mensajes privados están llenas en un <?php echo $percent_full; ?>%.</td>
</tr>
<tr>
...
<tr class="RowStyleB">
	<td class="SmallText al" colspan="3"><?php echo ($private_tools ? '<a href="/index.php?t=pmsg_dump&amp;'._rsid.'">Descargar volcado de mensajes</a>' : '&nbsp;' )  .'</td>
	<td colspan="3" class="SmallText ar nw">
		Carpeta:
		<select name="folder_id" onchange="document.forms[\'priv_frm\'].submit();">
		'.$select_options_cur_folder.'
		</select> <input type="submit" class="button" name="sel_folder" value="Ir" />
	</td>
</tr>
<tr>
	<th class="nw">Seleccionar <input type="checkbox" name="toggle" title="todos/ninguno" onclick="jQuery(\'input:checkbox\').prop(\'checked\', this.checked);" /> </th>
	<th>&nbsp;</th>
	<th class="wa"><a href="/index.php?t=pmsg&amp;folder_id='.$folder_id.'&amp;'._rsid.'&amp;o=subject&amp;s='.($o=='p.subject' && $s=='ASC' ? 'd' : 'a'); ?>" class="thLnk">Asunto</a></th>
	<th class="ac"><a href="/index.php?t=pmsg&amp;folder_id=<?php echo $folder_id; ?>&amp;<?php echo _rsid; ?>&amp;o=<?php echo ($folder_id == 3 ? 'alias2' : 'alias' )  .'&amp;s='.(($o=='u.alias' || $o=='u2.alias') && $s=='ASC' ? 'd' : 'a' )  .'" class="thLnk">'.($folder_id == 3 ? 'Destinatario' : 'Autor'); ?></a></th>
	<th class="ac"><a href="/index.php?t=pmsg&amp;folder_id=<?php echo $folder_id; ?>&amp;<?php echo _rsid; ?>&amp;o=post_stamp&amp;s=<?php echo ($o=='p.post_stamp' && $s=='ASC' ? 'd' : 'a'); ?>" class="thLnk">Hora</a></th>
	<th class="ac">Acción</th>
</tr>
