...
/* Print number of unread private messages in User Control Panel. */
	...
	if ($del) {
		...
		if (empty($_POST['confirm'])) {
			if ($data[2] != $data[4]) {
				$delete_msg = 'Está a punto de <span class="ErrorText">borrar</span> el mensaje titulado: <b>'.$data[3].'</b>';
			} else {
				$delete_msg = 'Está a punto de <span class="ErrorText">borrar</span> el <span class="ErrorText">tema completo</span> titulado: <b>'.$data[3].'</b>';
			}
			?> <!DOCTYPE html>
<html lang="es" dir="ltr">
<head>
...
<tr>
	<th>Confirmar el borrado del hilo o mensaje</th>
</tr>
	...
	<td class="GenText">
		<form method="post" action="/index.php?t=mmod">
			...
			<div class="mnavWarnTbl">
				<b>Opcional</b><br />Especifique una razón para borrar el mensaje, que se enviará al autor mediante un mensaje privado.<br />
				<textarea name="del_reason" style="width:98%; height:220px;"></textarea>
				<br />
				<label><input type="checkbox" name="del_inc_body" /> Incluir el cuerpo del mensaje eliminado que se enviará al autor.</label>
				<br /><br />
			</div>
			<br />
			<span class="fb">¿Desea continuar?</span><br />
			<input type="submit" class="button" name="YES" value="Sí" />
			&nbsp;&nbsp;&nbsp;
		...
		if (isset($_POST['YES'])) {
			...
			if ($if_not_pm) {
				...
				if ($body) {
					...
					$body = ($GLOBALS['FUD_OPT_1'] & 1024 ? filter_var($_POST['del_reason'], FILTER_SANITIZE_STRING).'' : $_POST['del_reason'].'' )  .'
--------------------------------------
Mensaje original
--------------------------------------
Asunto: '.$data[15].'
...
'.$body;
				...
				send_status_update($data[12], $data[13], $data[14], 'Su mensaje ha sido eliminado por el equipo de administración.', $body);
			}
			...
			if ($data[2] == $data[4]) {
				...
				if (strpos($usr->returnto, 'selmsg') === false) {
					if ($FUD_OPT_2 & 32768) {
						header('Location: /index.php/f/'. $data[0] .'/'. _rsidl);
					} else {
						header('Location: /index.php?t='. t_thread_view .'&'. _rsidl .'&frm_id='. $data[0]);
					}
					exit;
				} else {
					check_return($usr->returnto);
		...
		if (d_thread_view == 'tree') {
			if (!$data[5]) {
				if ($FUD_OPT_2 & 32768) {
					header('Location: /index.php/mv/tree/'. $data[1] .'/'. _rsidl);
				} else {
					header('Location: /index.php?t=tree&'. _rsidl .'&th='. $data[1]);
				}
			} else {
				if ($FUD_OPT_2 & 32768) {
					header('Location: /index.php/mv/tree/'. $data[1] .'/'. $data[5] .'/'. _rsidl);
				} else {
					header('Location: /index.php?t=tree&'. _rsidl .'&th='. $data[1] .'&mid='. $data[5]);
				}
			}
		} else {
			$prev_id = q_singleval(q_limit('SELECT id FROM fud30_msg WHERE thread_id='. $data[1] .' AND id < '. $data[2] .' ORDER BY id DESC', 1));
		
			if ($FUD_OPT_2 & 32768) {
				header('Location: /index.php/mv/msg/'. $data[1] .'/'. $prev_id .'/'. _rsidl);
			} else {
				header('Location: /index.php?t=msg&th='. $data[1] .'&'. _rsidl .'&goto='. $prev_id);
			}
		}
		exit;
