...
ses_update_status($usr->sid, 'Navegar en el calendario del foro');

$TITLE_EXTRA = ': Calendario';
...
function draw_calendar($year, $month, $size = 'large', $highlight_y = '', $highlight_m = '', $highlight_d = '') {
	...
	if ($size == 'large') {
		...
		$weekdays = array('D','L','Ma','Mi','J','V','S');
	}
	// WEEK START ON MONDAY: $weekdays = array('Lunes','Martes','Miércoles','Jueves','Viernes','Sábado', 'Domingo');
	...
	if ($GLOBALS['FUD_OPT_3'] & 268435456) {
		...
		if ($day == 0) {
			...
			while ($r = db_rowarr($c)) {
				$dd        = $r[0];
				$birthdays = $r[1];
				$events[ $year . $month . $dd ][] = convertPlural($cumpleaños, array(''.$cumpleaños.' cumpleaños',''.$cumpleaños.' cumpleaños'));
			}
		} else {
			...
			while ($r = db_rowarr($c)) {
				...
				$events[ $year . $mm . $dd ][] = 'Fecha de nacimiento: '.$user.' '.($age ? '('.convertPlural($edad, array(''.$edad.' año',''.$edad.' años')).' años de edad).' : '' ) ;
			}
		}
	}
...
<tr>
	<td class="ac" colspan="3">
		...
		<b>Ir a:</b><input type="hidden" name="t" value="cal" />
		<br />'.$select_month_control.' '.$select_year_control.' <input type="submit" name="submit" value="Ir" />
		</form>
	</td>
</tr>
' : ''); ?>
...
<tr>
	<td class="RowStyleB vt" width="55%">
		...
		'.($events_for_day ? '<ul>'.$events_for_day.'</ul>' : '<p>No hay eventos para este día.</p>' )  .'
		<br /><br />
		<form method="get" action="index.php">
		Ir a: <input type="hidden" name="t" value="cal" /><input type="hidden" name="view" value="'.$view.'" />
		'.$select_month_control.' '.$select_year_control.' 
		<input type="hidden" name="day" value="'.$day.'" /><input type="submit" name="submit" value="Ir" />
		</form>
	</td>
