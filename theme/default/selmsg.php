...
function path_info_lnk($var, $val)
{
	...
	ses_update_status($usr->sid, 'Recorriendo los <a href="/index.php?t=selmsg&amp;date=today&amp;rid='.$usr->id.'">mensajes de hoy</a>');
...
if (_uid) {
	...
	if ($unread_limit || $total) {
		...
		while ($r = db_rowobj($c)) {
			if ($prev_frm != $r->forum_id) {
				...
				$message_data .= '<tr>
	<th class="SelFS">
		Foro: <a class="thLnk" href="/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'"><span class="lg">'.$r->name.'</span></a>
	</th>
</tr>';
			...
			if ($prev_th != $r->thread_id) {
				...
				$message_data .= '<tr>
	<th class="SelTS">
		&nbsp;Hilo de discusión: <a class="thLnk" href="/index.php?t='.d_thread_view.'&amp;goto='.$r->id.'&amp;'._rsid.'#msg_'.$r->id.'">'.$r->thr_subject.'</a>
	</th>
</tr>';
			}
	...
	if (!$unread_limit && $total > $count) {
		...
		$pager = $message_data ? '<div class="GenText ac">[<a href="/index.php?'.$_SERVER['QUERY_STRING'].'" title="Mostrar más mensajes no leídos y marcar los mensajes actualmente visibles como leídos">más mensajes no leídos</a>]</div><img src="/blank.gif" alt="" height="3" />' : '';
	} else {
		$pager = '';
	}
	...
	if (!$message_data) {
		if (isset($_GET['unread'])) {
			$message_data = '<tr><th class="ac">No hay mensajes sin leer que coincidan con su consulta.</th></tr>';
			if (!$frm_id && !$th) {
			...
			$message_data = '<tr><th align="center">No hay mensajes</th></tr>';
		}
	}
...
<a href="/index.php?<?php echo $dt_opt; ?>" title="Activar/desactivar el filtro de mensajes de hoy (sólo mostrará mensajes publicados hoy)">Mensajes de hoy <?php echo (isset($_GET['date']) ? '<span class="selmsgInd">(<span class="GenTextRed">activado</span>)</span>' : '<span class="selmsgInd">(desactivado)</span>' )  .'</a>
'.(_uid ? '&nbsp;| <a href="/index.php?'.$un_opt.'" title="Activar/desactivar el filtro de mensajes no leídos (sólo mostrará mensajes no leídos)">Mensajes no leídos '.(isset($_GET['unread']) ? '<span class="selmsgInd">(<span class="GenTextRed">activado</span>)</span>' : '<span class="selmsgInd">(desactivado)</span>' )  .'</a>' : ''); ?>
<?php echo (_uid ? '&nbsp;| <a href="/index.php?'.$frm_opt.'" title="Activar/desactivar el filtro de foros suscritos (sólo mostrará mensajes publicados en los foros a los que esté suscrito)">Foros suscritos '.(isset($_GET['sub_forum_limit']) ? '<span class="selmsgInd">(<span class="GenTextRed">activado</span>)</span>' : '<span class="selmsgInd">(desactivado)</span>' )  .'</a>' : ''); ?>
<?php echo (_uid ? '&nbsp;| <a href="/index.php?'.$th_opt.'" title="Activar/desactivar el filtro de hilos de discusión suscritos (sólo mostrará mensajes publicados en los hilos a los cuales esté suscrito)">Hilos de discusión suscritos '.(isset($_GET['sub_th_limit']) ? '<span class="selmsgInd">(<span class="GenTextRed">activado</span>)</span>' : '<span class="selmsgInd">(desactivado)</span>' )  .'</a>' : ''); ?>
&nbsp;| <a href="/index.php?<?php echo $rp_opt; ?>" title="Activar/desactivar el filtro de mensajes no respondidos (sólo mostrará mensajes que no tienen respuestas)">Mensajes no respondidos <?php echo (isset($_GET['reply_count']) ? '<span class="selmsgInd">(<span class="GenTextRed">activado</span>)</span>' : '<span class="selmsgInd">(desactivado)</span>'); ?></a>
