...
}function read_msg_body($off, $len, $id)
{
	...
	if (isset($_POST['btn_submit'], $_POST['nlogin'], $_POST['cpasswd']) && is_string($_POST['nlogin'])) {
		...
		if (__fud_real_user__ != $r->id || !((empty($r->salt) && $r->passwd == md5((string)$_POST['cpasswd'])) || $r->passwd == sha1($r->salt . sha1((string)$_POST['cpasswd'])))) {
			$ruser_error_msg = 'La contraseña no es válida';
		} else if (strlen($_POST['nlogin']) < 4) {
			$ruser_error_msg = 'El nombre de usuario que has seleccionado es demasiado corto. Los nombres de usuarios deben tener al menos 2 caracteres.';
		} else if (is_login_blocked($_POST['nlogin'])) {
			$ruser_error_msg = 'Este nombre de usuario no está permitido.';
		} else if (get_id_by_login($_POST['nlogin'])) {
			$ruser_error_msg = 'Los nombres de usuario del foro deben ser únicos. Ya existe un usuario con ese nombre.';
		} else {
	...
	$TITLE_EXTRA = ': Formulario de cambio de identificación';
...
<tr>
	<th colspan="2">Cambiar tu identificación en el foro</th>
</tr>
<tr class="RowStyleB">
	<td>Identificación actual:</td>
	<td><?php echo filter_var($usr->login, FILTER_SANITIZE_STRING); ?></td>
</tr>
<tr class="RowStyleB">
	<td>Nueva identificación:</td>
...
<tr class="RowStyleB">
	<td align="right" colspan="2"><input type="submit" class="button" value="Ir" name="btn_submit" /></td>
</tr>
</table>
</div>
<?php echo _hs; ?>
