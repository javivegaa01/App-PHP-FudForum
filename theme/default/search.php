...
function fetch_search_cache($qry, $start, $count, $logic, $srch_type, $order, $forum_limiter, &$total)
{
	...
	if (!($total = q_singleval('SELECT count(*) FROM fud30_search_cache WHERE srch_query=\''. $qry_lck .'\' AND query_type='. $qt))) {
		q('INSERT INTO fud30_search_cache (srch_query, query_type, expiry, msg_id, n_match, score) '. 
		  q_limit('SELECT \''. $qry_lck .'\', '. $qt .', '. __request_timestamp__ .', msg_id, count(*) as word_count, sum(frequency) as frequency FROM fud30_search s INNER JOIN fud30_'. $tbl .' i ON i.word_id=s.id WHERE word IN('. $qr .') GROUP BY msg_id ORDER BY frequency DESC', 
		          5000, 0));
	}
...
function trim_body($body)
{
	...
	$search_options = tmpl_draw_radio_opt('field', "all\nsubject", "En todo el mensaje\nSolo el asunto", $field, '&nbsp;&nbsp;');
	$logic_options  = tmpl_draw_select_opt("AND\nOR", "Y\nO", $search_logic);
	$sort_options   = tmpl_draw_select_opt("REL\nDESC\nASC", "Relevancia:\nMás recientes primero\nMás antiguos primero", $sort_order);
	$attach_options = tmpl_draw_select_opt("0\n1", "Con o sin\nCon adjuntos", $attach);

	$TITLE_EXTRA = ': Buscar '.filter_var($srch, FILTER_SANITIZE_STRING);

	ses_update_status($usr->sid, 'Buscando mensajes');
...
<tr>
	<th class="wa ac">Sin resultados</th>
</tr>
</table>';
			$page_pager = '';
...
//}
	...
	<td class="'.alt_var('search_alt','RowStyleA','RowStyleB').'">
		<b>Foro</b>: <a href="/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'">'.$r->forum_name.'</a> &laquo;&raquo;
		<b>Publicado el</b>: <span class="DateText">'.strftime('%a, %d %B %Y %H:%M', $r->post_stamp).'</span> &laquo;&raquo;
		<b>De:</b> '.(!empty($r->poster_id) ? '<a href="/index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.filter_var($r->alias, FILTER_SANITIZE_STRING).'</a>' : $GLOBALS['ANON_NICK'].'' ) .'<br />
		...
		'.(($is_a || $r->md) ? '
		<div class="ModOpt">Opciones del moderador: <a href="/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r->thread_id.'&amp;del='.$r->id.'">Borrar</a>
'.($r->is_rootm ? '
 | <a href="javascript://" onclick="window_open(\'/index.php?t=mvthread&amp;'._rsid.'&amp;th='.$r->thread_id.'\', \'th_move\', 300, 400);">Trasladar</a> | <a href="/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r->thread_id.'&amp;'.($r->is_lckd ? 'unlock' : 'lock' )  .'=1&amp;SQ='.$GLOBALS['sq'].'">'.($r->is_lckd ? 'Desbloquear el hilo de discusión' : 'Bloquear hilo de discusión' )  .'</a>
' : '' )  .'
</div>
		' : '' )  .'
	</td>
...
<tr>
	<th> </th>
	<th>Se encontraron '.$total.' resultados para su búsqueda</th>
</tr>
'.$search_data.'
</table>';
...
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr>
	<th>Búsqueda en el foro</th><th class="wa">Opciones de búsqueda</th>
</tr>
<tr class="RowStyleA">
	<td class="vt nw"><input spellcheck="true" type="search" name="srch" tabindex="1" value="<?php echo filter_var($srch, FILTER_SANITIZE_STRING); ?>" /> <input type="submit" tabindex="2" class="button" name="btn_submit" value="Buscar" /><br /><span class="SmallText"><?php echo $search_options; ?></span></td>
	<td class="RowStyleB SmallText">
		<div class="sr">Buscar en el foro:<br /><select class="SmallText" name="forum_limiter"><option value="">Buscar en todos los foros</option><?php echo $forum_limit_data; ?></select></div>
		<div class="sr">Adjuntos:<br /><select class="SmallText" name="attach"><?php echo $attach_options; ?></select></div>
		<div class="sr">Lógica de búsqueda:<br /><select class="SmallText" name="search_logic"><?php echo $logic_options; ?></select></div>
		<div class="sr">Ordenar por:<br /><select class="SmallText" name="sort_order"><?php echo $sort_options; ?></select></div>
		<div class="sr">Filtrar por usuario:<br /><input class="SmallText" type="text" value="<?php echo filter_var($author, FILTER_SANITIZE_STRING); ?>" name="author" /></div>
	</td>
</tr>
</table>
</form>
...
<script>
	...
	highlightSearchTerms("<?php echo filter_var($_GET['srch'], FILTER_SANITIZE_STRING); ?>");
</script>
