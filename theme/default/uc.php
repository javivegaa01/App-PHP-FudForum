...
<tr>
	...
	while ($r = db_rowarr($c)) {
		...
		$obj = new stdClass();
		$obj->login = $r[1];
		$uc_online = (!$r[3] && ($r[2] + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) ? '<img src="/theme/default/images/online.png" alt="'.$obj->login.' está actualmente en el foro" title="'.$obj->login.' está actualmente en el foro" />' : '<img src="/theme/default/images/offline.png" alt="'.$obj->login.' no está actualmente en el foro" title="'.$obj->login.' no está actualmente en el foro" />';
...
De: '.($r->alias ? '<a href="/index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.filter_var($r->alias, FILTER_SANITIZE_STRING).'</a>' : $GLOBALS['ANON_NICK'].'' ) : '' ) .'</td>
	...
	if ($uc_sub_forum) {
		$uc_sub_forum = '<tr>
        <th class="wa">Categoría &raquo; Foro</th>
	<th class="nw">Mensajes</th>
        <th class="nw">Hilos de discusión</th>
        <th class="nw">Último mensaje</th>
</tr>
'.$uc_sub_forum;
	}
	unset($c);
