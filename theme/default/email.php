...
}class fud_smtp
{
	...
	if (!($FUD_OPT_2 & 1073741824)) {
		error_dialog('ERROR: No fue posible enviar el correo', 'El sistema de correo electrónico ha sido desactivado por el administrador.');
	}
...
function mail_check()
{
	if (!strlen(trim($_POST['tx_body']))) {
		set_err('tx_body', 'Se necesita un cuerpo del mensaje');
	}

	if (!strlen(trim($_POST['tx_subject']))) {
		set_err('tx_subject', 'Se necesita un asunto');
	}

	if (!strlen(trim($_POST['tx_name']))) {
		set_err('tx_name', 'Se necesita un nombre');
	} else if (!q_singleval('SELECT id FROM fud30_users WHERE alias='. _esc(htmlspecialchars($_POST['tx_name'])))) {
		set_err('tx_name', 'El usuario no es válido');
	}
	...
	if (isset($_POST['btn_submit'])) {
		if (!mail_check()) {
			if (!($email = q_singleval('SELECT email FROM fud30_users WHERE alias='. _esc(htmlspecialchars($_POST['tx_name'])) .' AND '. q_bitand('users_opt', 16) .' > 0'))) {
				error_dialog('ERROR: No fue posible enviar el correo', 'El sistema no pudo enviar un correo a '.filter_var($_POST['tx_name'], FILTER_SANITIZE_STRING).' porque no fue capaz de encontrar su dirección de correo electrónico');
			}
	...
	/* start page */
	$TITLE_EXTRA = ': Formulario de correo electrónico';
...
<tr class="RowStyleB">
	<td class="GenText nw vt">Cuerpo: <?php echo get_err('tx_body'); ?></td>
...
<tr class="RowStyleC">
	<td class="ar" colspan="2"><input type="submit" class="button" name="btn_submit" tabindex="3" value="Correo electrónico" /></td>
</tr>
