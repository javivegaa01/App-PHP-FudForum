...
}function read_msg_body($off, $len, $id)
{
	...
	ses_update_status($usr->sid, 'Restablecer contraseña');
	...
	if (isset($_GET['reset_key'])) {
		if (($ui = db_saq('SELECT email, login, id FROM fud30_users WHERE reset_key='. _esc((string)$_GET['reset_key'])))) {
			...
			send_email($NOTIFY_FROM, $ui[0], 'Restablecer contraseña', 'Hola.\n\nTal como solicitó, le remitimos su nombre de usuario y contraseña.\nAquí está su información de inicio de sesión:\n\nUsuario: '.$ui[1].'\nContraseña: '.$passwd.'\n\nRecuerde que su contraseña ha sido actualizada al valor anterior. Si desea cambiar su contraseña puede hacerlo mediante el panel de control de usuario en:\nhttp://www.javiervega-fudforum.org/index.php?t=register\n\n\n\nSi recibió este mensaje por error, por favor, ignórelo. Si está recibiendo múltiples copias de este correo, y no las ha solicitado, por favor, póngase en contacto con el administrador del foro en '.$GLOBALS['ADMIN_EMAIL'].'\n\nEsta solicitud se realizó desde: '.$_SERVER['REMOTE_ADDR'].'.\n\n');

			// Message to display on login screen.
			ses_putvar((int)$usr->sid, 'resetmsg=Se le ha enviado su contraseña por correo electrónico. Debería recibirla en unos minutos.');
		...
		error_dialog('ERROR', 'La clave de actualización de contraseña no es válida');
	}
...
<tr>
	<th colspan="2">Recordatorio de contraseña</th>
</tr>
<tr>
	<td colspan="2" class="RowStyleA GenText">Su contraseña será actualizada y se le enviará, o en caso que no haya confirmado su correo electrónico, la solicitud de confirmación de correo electrónico le será reenviada.</td>
</tr>
<tr class="RowStyleB">
	<td class="GenText">Correo electrónico:</td>
	<td><?php echo $no_such_email; ?><input type="email" name="email" value="<?php echo filter_var($email, FILTER_SANITIZE_STRING); ?>" /></td>
</tr>
<tr class="RowStyleA">
	<td class="nw ar" colspan="2"><input type="submit" class="button" name="reset_passwd" value="Actualizar contraseña" /></td>
</tr>
</table>
<?php echo _hs; ?>
</form>
