...
/* Print number of unread private messages in User Control Panel. */
	...
	if ($final_del) { /* Remove threads, one by one. */
		...
		foreach (array($perms[0], $_POST['forum_id']) as $v) {
			$mid = (int) q_singleval('SELECT MAX(last_post_id) FROM fud30_thread t INNER JOIN fud30_msg m ON t.root_msg_id=m.id WHERE t.forum_id='. $v .' AND t.moved_to=0 AND m.apr=1');
		...
		header('Location: /index.php?t=merge_th&frm_id='. $perms[0] .'&new_title='. urlencode($new_title) .'&sel_th='. serialize($sel_th) .'&'. _rsidl);
		exit;
	}
...
<input type="hidden" name="_sel[]" value="'.$k.'" />';
	...
	<td class="RowStyleC" style="padding-left: '.($tabw = ($i[0] * 10 + 2)).'px">'.$i[1].'</td>
			...
			if ($r[3] || $is_a || $r[4] & 8192) {
				$table_data .= '<tr>
	<td class="RowStyleB" style="padding-left: '.$tabw.'px"><label><input type="radio" name="forum_id" value="'.$r[1].'" />'.$r[0].'</label></td>
</tr>';
			}
		}
		unset($c);
...
<div align="center">
<b><?php echo (!empty($_POST['del_sel_all']) ? 'Borrar hilos' : 'Trasladar Temas'); ?>:</b><br />
...
<input type="submit" name="del_conf" value="Sí" /> 
' : '
<table cellspacing="0" cellpadding="3" class="DialogTable dashed">
<tr>
	<th>Foro de destino:</th>
</tr>
'.$table_data.'
</table>
<input type="submit" name="NO" value="Cancelar" /> 
<input type="hidden" name="mov_sel_all" value="1" />
<input type="submit" name="submit" value="Trasladar Temas" /> 
'); ?>
</div>
</form>
