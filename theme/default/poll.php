...
function reverse_fmt($data)
{
	...
	$TITLE_EXTRA = ': Editor de la encuesta';



	$pl_expiry_date_data = tmpl_draw_select_opt("0\n3600\n21600\n43200\n86400\n259200\n604800\n2635200\n31536000", "Ilimitada\n1 hora\n6 horas\n12 horas\n1 día\n3 días\n1 semana\n1 mes\n1 año", $pl_expiry_date);
	...
	<td align="right" valign="top" colspan="2" class="GenText"><label><input type="checkbox" name="pl_smiley_disabled" value="Y" checked="'.$checked.'" />Desactivar los emoticonos</label></td>
</tr>';
	} else {
	...
	<td class="nw">
		[<a href="/index.php?t=poll&amp;frm_id='.$frm_id.'&amp;'._rsid.'&amp;pl_id='.$pl_id.'&amp;pl_optedit='.$k.'">Editar</a>]
		[<a href="/index.php?t=poll&amp;frm_id='.$frm_id.'&amp;pl_id='.$pl_id.'&amp;del_id='.$k.'&amp;'._rsid.'">Borrar</a>]
	</td>
</tr>';
		}
...
<tr>
	<td class="GenText">Agregar opción:</td>
	<td class="ar"><input type="text" spellcheck="true" name="pl_option" value="'.filter_var($pl_option, FILTER_SANITIZE_STRING).'" />
	'.$pl_smiley_disabled_chk.'
<tr>
	<td colspan="2" class="ar">'.(!isset($_GET['pl_optedit']) ? '<input type="submit" class="button" name="pl_add" onclick="return check_submission();" value="Agregar opción" />' : '<input type="hidden" name="pl_option_id" value="'.$pl_option_id.'" />
<input type="submit" class="button" name="pl_upd" onclick="return check_submission();" value="Actualizar opción" />' ) .'</td>
</tr>
<tr>
	<td colspan="2">
		<table>
...
<tr>
	<td class="GenText">Nombre de la encuesta:</td>
	<td><input tabindex="1" type="text" spellcheck="true" name="pl_name" value="<?php echo filter_var($pl_name, FILTER_SANITIZE_STRING); ?>" /></td>
</tr>
<tr>
	<td class="GenText">Límite de tiempo:</td>
	<td><select tabindex="2" name="pl_expiry_date"><?php echo $pl_expiry_date_data; ?></select></td>
</tr>
<tr>
	<td class="GenText">Votos máximos:</td>
	<td><select tabindex="3" name="pl_max_votes"><?php echo $pl_max_votes_data; ?></select></td>
</tr>
<tr>
	<td colspan="2" class="ar"><?php echo (!$pl_id ? '<input tabindex="5" type="submit" class="button" name="pl_submit" value="Crear encuesta" />' : '<input tabindex="5" type="submit" class="button" name="pl_submit" value="Actualizar encuesta" />'); ?></td>
</tr>
</table>
<br />
...
<br />
<div class="ar">
<?php echo (!$pl_id ? '<input type="button" tabindex="4" class="button" onclick="window.opener.document.forms[\'post_form\'].pl_id.value=\'0\'; window.opener.document.forms[\'post_form\'].submit(); window.close();" value="Crear" />' : '<input type="button" tabindex="4" class="button" onclick="window.opener.document.forms[\'post_form\'].pl_id.value='.$pl_id.'; window.opener.document.forms[\'post_form\'].submit(); window.close();" value="Actualizar" />'); ?>
</div>
</form>
<script>
function check_submission()
{
	...
	if (!a.length) {
		if (document.forms['poll_submit'].pl_add && document.forms['poll_submit'].pl_add.value.length) {
			alert("No se puede añadir una opción vacía.");
			return false;
		} else if (document.forms['poll_submit'].pl_upd && document.forms['poll_submit'].pl_upd.value.length) {
			return confirm("Si quiere borrar esta opción, pulse Aceptar");
		}
	}
	return true;
}
