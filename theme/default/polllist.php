...
if (_uid) {
	...
	while ($obj = db_rowobj($c)) {
		...
		if ($obj->owner && (!($obj->users_opt & 32768) || $is_a) && $FUD_OPT_2 & 32) {
			$online_indicator = $obj->last_visit > __request_timestamp__ ? '<img src="/theme/default/images/online.png" title="'.$obj->login.' está actualmente en el foro" alt="'.$obj->login.' está actualmente en el foro" />&nbsp;' : '<img src="/theme/default/images/offline.png" title="'.$obj->login.' no está actualmente en el foro" alt="'.$obj->login.' no está actualmente en el foro" />&nbsp;';
		} else {
			$online_indicator = '';
		}
