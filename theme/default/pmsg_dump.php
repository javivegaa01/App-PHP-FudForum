...
}class fud_smtp
{
	...
	$c = q('SELECT p.subject, p.post_stamp, p.foff, p.length, u.alias
		FROM fud30_pmsg p INNER JOIN fud30_users u ON p.ouser_id=u.id
		WHERE p.duser_id='. _uid .' AND p.fldr IN(1, 2, 3) ORDER BY p.id');

	$pm_data = '';
	while ($obj = db_rowobj($c)) {
		$pm_data .= '<table cellspacing="0" cellpadding="0" class="MsgTable">
<tr>
	<td class="MsgR1 al vt"><b>Asunto:</b> '.$obj->subject.'</td>
	<td class="MsgR1 ar vt DateText"><b>De:</b> '.filter_var($obj->alias, FILTER_SANITIZE_STRING).' <b>el</b> '.strftime('%a, %d %B %Y %H:%M', $obj->post_stamp).'</td>
</tr>
<tr>
	<td class="MsgR3" colspan="2">'.read_pmsg_body($obj->foff, $obj->length).'
</tr>
</table>';	
	}
	unset($c);
...
<body>
	<div class="content">
		<h1>Mensajes privados para <?php echo $usr->alias; ?></h1>
		<?php echo $pm_data; ?>
		<small>Exportado: <?php echo strftime('%a %b %d %H:%M:%S %Z %Y', __request_timestamp__); ?></small>
	</div>
</body>
</html>