...
function error_check()
{
	if (empty($_POST['login']) || !strlen($_POST['login'] = trim((string)$_POST['login']))) {
		login_php_set_err('login', 'Debe introducir un nombre de usuario');
	}

	if (empty($_POST['password']) || !strlen($_POST['password'] = trim((string)$_POST['password']))) {
		login_php_set_err('password', 'Debe introducir una contraseña');
	}
	...
	if ($usr_d || isset($_POST['login']) && !error_check()) {
		...
		if (!$usr_d && !($usr_d = db_sab('SELECT last_login, id, passwd, salt, login, email, users_opt, ban_expiry, ban_reason FROM fud30_users WHERE login='. _esc($_POST['login'])))) {
			...
			login_php_set_err('login', 'El nombre de usuario o la contraseña no son válidos');
			...
			login_php_set_err('login', 'Este foro permite un intento de login cada '.$MIN_TIME_BETWEEN_LOGIN.' segundos, por favor espere '.($usr_d->last_login + $MIN_TIME_BETWEEN_LOGIN - __request_timestamp__).' segundos antes de intentarlo de nuevo.');
		...
		if ($GLOBALS['_ERROR_'] != 1) {
			...
			if (!($usr_d->users_opt & 131072)) {
				error_dialog('ERROR: su cuenta aún no ha sido confirmada', 'Aún no has confirmado tu cuenta por correo electrónico.<br /><table border="0"><tr><td><ol><li>Si no has recibido un correo de confirmación, <a href="/index.php?t=reset&amp;email='.$usr_d->email.'&amp;S='.$ses_id.'">haz clic aquí</a><li>Si '.$usr_d->email.' no es tu dirección de correo electrónico correcta, <a href="/index.php?t=register&amp;S='.$ses_id.'">indica tu dirección de correo electrónico</a>.</ol></td></tr></table>', null, $ses_id);
			}
			if ($usr_d->users_opt & 2097152) {
				error_dialog('La cuenta no está aprobada', 'El administrador de los foros ha elegido confirmar manualmente cada cuenta nueva antes de activarla. Su cuenta todavía no ha sido confirmada, y por lo tanto no podrá acceder a algunas de las características disponibles para los miembros registrados hasta que su cuenta esté aprobada.', null, $ses_id);
			}

			if (!empty($_POST['adm']) && $usr_d->users_opt & 1048576) {
				header('Location: /adm/index.php?S='. $ses_id .'&SQ='. $new_sq);
				exit;
			}
	...
	ses_update_status($usr->sid, 'Formulario de inicio de sesión', 0, 0);
	$TITLE_EXTRA = ': Formulario de inicio de sesión';
...
<tr>
	<th colspan="3">Formulario de inicio de sesión</th>
</tr>
<tr>
	<td class="RowStyleA GenText" colspan="3">
		'.(isset($reset_login_notify) ? $reset_login_notify.'' : '
		No ha iniciado la sesión. Esto puede deberse a una de varias razones:
		<ol>
			<li class="GenText">Su cookie ha expirado, y debe iniciar sesión para renovarla.</li>
			<li class="GenText">No tiene permiso para acceder al recurso solicitado como usuario anónimo. Debe iniciar sesión para poder hacerlo.</li>
		</ol>'); ?>
	</td>
</tr>
<tr class="RowStyleB">
	<td class="GenText">Iniciar sesión</td>
	<td><?php echo login_php_get_err('login'); ?><input type="text" tabindex="1" name="login" /></td>
	<td class="nw"><?php echo ($FUD_OPT_1 & 2 ? '<a href="/index.php?t=register&amp;'._rsid.'">¿Quiere registrarse?</a>' : ''); ?></td>
</tr>
<tr class="RowStyleA">
	<td class="GenText">Contraseña:</td>
	<td><?php echo login_php_get_err('password'); ?><input type="password" tabindex="2" name="password" /></td>
	<td class="nw"><?php echo ($FUD_OPT_4 & 2 ? '<a href="/index.php?t=reset&amp;'._rsid.'">¿Olvidó su contraseña?</a>' : ''); ?></td>
</tr>
<?php echo ($FUD_OPT_1 & 128 ? '<tr class="RowStyleB">
	<td colspan="3" class="al">
		<label><input type="checkbox" name="use_cookie" value="Y" checked="checked" /> Usar cookies<br /><span class="SmallText">Si estás usando un terminal público, como por ejemplo una computadora en una biblioteca, escuela, o cibercafé, se recomienda que deshabilite esta opción para una mayor seguridad. <br />Si dejas esta opción seleccionada entonces estarás automáticamente iniciando sesión en el foro la próxima vez que nos visites.</span></label>
	</td>
</tr>' : ''); ?>
<tr>
	<td colspan="3" class="RowStyleA ar"><input type="submit" class="button" tabindex="3" value="Iniciar sesión" /></td>
</tr>
